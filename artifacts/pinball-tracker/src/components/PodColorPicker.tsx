...
import { POD_PALETTE, normalizePodColor, nearReservedColor } from '../lib/podColor';
 ...
 * what's typed normalizes. Warns (doesn't block) when the pick is close to a reserved chart color,
 * since a pod line that looks like "You" or "All other players" would misread on a chart.
 */
export default function PodColorPicker({ value, onChange, idPrefix }: {
  ...
  const reservedClash = nearReservedColor(value, {
    'You': hslToHex(colors.username),
    'All other players': hslToHex(colors.field),
  });
  ...
  return (
    <div>
      ...
      {!draftValid && <p className="text-xs text-red-400 mt-1.5">Use a hex color like #60a5fa.</p>}
      {reservedClash && (
        <p className="flex items-center gap-1.5 text-xs text-amber-400 mt-1.5">
          ...
          Looks a lot like the “{reservedClash}” color on charts — consider another.
        </p>
      )}
    </div>
  );
