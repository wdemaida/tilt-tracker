...
// (endpoint ep-late-mouse-at8antth) while this is still on fix/pm-etiquette. Production is a
...
if (!host.startsWith(DEV_ENDPOINT)) {
  console.error(`Refusing to run: DATABASE_URL is not the Neon dev branch (${DEV_ENDPOINT}). See the guard comment in migrate16.ts.`);
  process.exit(1);
}
