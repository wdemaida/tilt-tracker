...
export const TARGET_RAW_BYTES = Math.floor((ANTHROPIC_MAX_BASE64_BYTES * 3) / 4 * 0.97);
 ...
 */
export async function fitUnderAnthropicLimit(
  buffer: Buffer,
  mimeType: string,
  // Multi-photo requests share Anthropic's request-size budget, so the caller can ask for less than the
  // per-image ceiling (see upload.ts).
  targetRawBytes: number = TARGET_RAW_BYTES,
): Promise<{ buffer: Buffer; mimeType: string }> {
  const TARGET = Math.min(targetRawBytes, TARGET_RAW_BYTES);
  if (buffer.length <= TARGET) return { buffer, mimeType };
  ...
  if (working.length <= TARGET) return { buffer: working, mimeType: 'image/jpeg' };

  for (let quality = 80; quality >= 40; quality -= 10) {
    const out = await sharp(working).jpeg({ quality, mozjpeg: true }).toBuffer();
    if (out.length <= TARGET) return { buffer: out, mimeType: 'image/jpeg' };
  }
