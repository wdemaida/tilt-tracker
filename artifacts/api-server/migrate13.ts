...
// (endpoint ep-late-mouse-at8antth) while Pods is still on feature/pods. Production is a different
// endpoint, so running this from the production checkout (whose .env points at prod) aborts here.
// Remove this block deliberately at ship time, when the migration is meant to hit production.
const DEV_ENDPOINT = 'ep-late-mouse-at8antth';
const host = new URL(process.env.DATABASE_URL!).hostname;
if (!host.startsWith(DEV_ENDPOINT)) {
  console.error(`Refusing to run: DATABASE_URL is not the Neon dev branch (${DEV_ENDPOINT}). See the guard comment in migrate13.ts.`);
  process.exit(1);
}
